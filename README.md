...
- 系統備份：管理員可手動建立 PostgreSQL JSON 備份、查看紀錄、還原與刪除；後端每日凌晨自動備份並保留最近 7 天
- 響應式版面，支援桌面與手機瀏覽
...
- Environment variables:
  ...
  - `BACKUP_DIR=<optional local backup path>`
  - `BACKUP_TIMEZONE=Asia/Taipei`

注意：Render 免費方案的 local filesystem 可能不是永久儲存。系統目前優先支援 local backups folder，並已在備份 metadata 預留 Google Drive / S3 / Cloudinary 擴充欄位。
